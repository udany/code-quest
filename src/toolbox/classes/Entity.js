...
export class Entity extends Emitter {
	constructor(a) {
		...
		this.on('get', this._loadLazyAttribute);
	}

	async _loadLazyAttribute(attribute) {
		const lazy = this._lazy;

		if (lazy && lazy.hasOwnProperty(attribute.name) && lazy[attribute.name] instanceof Function) {
			if (!this._lazyLoaded) this._lazyLoaded = {};

			if (!this._lazyLoaded[attribute.name]) {
				attribute.Set(this, await lazy[attribute.name]());
				this._lazyLoaded[attribute.name] = true;
			}
		}
	}
...
export class ObjectEntityAttribute {
	...
	define(obj) {
		...
		if (this.watchable()) {
			...
			descriptor.set = function (newValue) {
				...
				if (oldValue instanceof Array) {
					obj.emit('list-remove', [attribute, ...oldValue.filter(x => newValue.indexOf(x) !== -1)]);
					obj.emit('list-add', [attribute, ...newValue.filter(x => oldValue.indexOf(x) !== -1)]);
		...
		if (this.descriptorEdit() instanceof Function) {
			descriptor = this.descriptorEdit()(descriptor);
		}

		if (descriptor && obj[this.name] === undefined) {
	...
	/**
	 * @param {T} [val]
	 * @return {ObjectEntityAttribute|T}
	 */
	default(val) {
		let r = this._setOrReturnKey('_default', val);
	...
	descriptorEdit(val) {
		return this._setOrReturnKey('_descriptorEdit', val);
	}
...
/// == String
/**
 * @class StringEntityAttribute
 * @extends {ObjectEntityAttribute<String>}
 */
export class StringEntityAttribute extends ObjectEntityAttribute {
	...
	Get(obj) {
		return obj[this.name] === null ? (this.nullable() ? null : this.default()) : obj[this.name].toString();
	}

	Set(obj, val) {
		obj[this.name] = val === null ? (this.nullable() ? null : this.default()) : val.toString();
	}
}
...
export class IntegerEntityAttribute extends ObjectEntityAttribute {
	...
	Get(obj) {
		...
		return isNaN(val) ? (this.nullable() ? null : this.default()) : val;
	}

	Set(obj, val) {
		val = parseInt(val, 10);
		obj[this.name] = isNaN(val) ? (this.nullable() ? null : this.default()) : val;
	}
}
...
export class EntityEntityAttribute extends ObjectEntityAttribute {
	 ...
	 */
	baseClass(val) {
		return this._setOrReturnKey('_baseClass', val);
	}

	_entityFromPlainObject(plain) {
		let obj;

		if (this.defaultEntity) {
			if (plain instanceof this.defaultEntity) {
				obj = plain;
			} else {
				obj = plain ? new this.defaultEntity(plain) : null;
			}
		} else {
			if (plain instanceof Entity) {
				obj = plain;
			} else {
				obj = plain ? Entity.FromPlainObject(plain) : null;
			}
		}

		if (this.baseClass() && !(obj instanceof this.baseClass())) {
			console.error(`Unexpected class for the value of ${this.name}, expected ${this.baseClass().name}`, obj);
			obj = null;
		}
	...
	Set(obj, val) {
		...
		obj[this.name] = this._entityFromPlainObject(dataArray);
	}
...
export class EntityListEntityAttribute extends EntityEntityAttribute {
	...
	Set(obj, val) {
		...
		dataArray = dataArray.map(x => this._entityFromPlainObject(x));
...
Entity.Attributes.EntityMap = EntityMapEntityAttribute;
...
Entity.Attributes.EnumList = EnumListEntityAttribute;
